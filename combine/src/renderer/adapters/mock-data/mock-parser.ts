/**
 * Независимая копия алгоритма ParserService (combine/src/core/parser/parser.service.ts) для mockAdapter.
 *
 * ПОЧЕМУ НЕ ИМПОРТИРУЕМ ОРИГИНАЛ: core/parser/parser.service.ts (и core/queue/build-items.ts) на момент
 * написания не проходят собственный typecheck (одинаково падают и typecheck:node, и typecheck:web —
 * т.е. это НЕ вызвано изменениями renderer-агента): `computeStats`/`flattenToTasks` используют
 * `if (type==='verb_group'||type==='phrase_group') {...} else if (type==='vocabulary') {...} else {...}`,
 * а TS не сужает финальный `else` до `BlockStoryJson`, оставляя `BlockGroupsJson | BlockStoryJson` —
 * известная особенность control-flow narrowing, когда дискриминант одного члена union сам является
 * объединением строковых литералов (`'verb_group' | 'phrase_group'`). Т.к. combine/src/core/** — чужая
 * зона (правит параллельный агент, конфликты недопустимы), мы не можем править файл — вместо этого здесь
 * везде используются ЯВНЫЕ ветки (`else if (type === 'story')`), которые сужаются корректно.
 *
 * Логика ниже 1:1 повторяет правила разбора из docs/SPEC_COMBINE.md §2 и оригинальный алгоритм; переиспользуются
 * безопасные (без этого бага и без Node-зависимостей) хелперы core: front-matter.ts и util/slug.ts.
 */
...
import type {
  ...
  ParseResult,
  ParserStats
} from '../../../core/types/parsed-lesson'
import { emptyStats } from '../../../core/types/parsed-lesson'
import type { BlockType } from '../../../core/types/lesson-json'
import { extractFrontMatter } from '../../../core/parser/front-matter'
import { pad2, slugify } from '../../../core/util/slug'
...
export function parseLessonText(raw: string): ParseResult {
  ...
  return { lesson, errors, warnings, stats: computeLessonStats(lesson) }
}

/** Аналог core computeStats, но с явными ветками (см. комментарий в шапке файла — обходим TS-баг). */
export function computeLessonStats(lesson: ParsedLesson): ParserStats {
  let phraseCount = 0
  let vocabCount = 0
  let storyCount = 0
  let charactersEs = 0
