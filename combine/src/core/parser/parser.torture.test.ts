...
describe('ParserService — направленные edge-cases', () => {
  ...
  describe('лишний "|" внутри строки фразы ("ES | RU | лишнее")', () => {
    it('ХАРАКТЕРИЗАЦИЯ фактического поведения: НЕ считается ошибкой — второй "|" молча остаётся частью RU (см. НАЙДЕН БАГ ниже)', () => {
      const raw = '#TOPIC 1 | Тема\n##BLOCK vocabulary | Лексика\nHola | Mundo | Extra\n'
      const result = parser.parse(raw)
      // Фактическое (не обязательно желаемое) поведение: splitOnce() режет только по ПЕРВОМУ "|",
      // остаток строки — включая второй "|" — целиком уходит в RU без какой-либо проверки/ошибки.
      expect(result.errors).toEqual([])
      if (result.lesson?.blocks[0].type === 'vocabulary') {
        expect(result.lesson.blocks[0].words[0]).toMatchObject({ es: 'Hola', ru: 'Mundo | Extra' })
      } else {
    ...
    it.skip('БАГ-репро: "ES | RU | лишнее" ДОЛЖНО давать структурированную ошибку с номером строки, а не тихо склеивать RU="Mundo | Extra"', () => {
      const raw = '#TOPIC 1 | Тема\n##BLOCK vocabulary | Лексика\nHola | Mundo | Extra\n'
      const result = parser.parse(raw)
...
describe('ParserService — edge-case: группа/vocabulary-блок с 100+ фразами (переполнение id)', () => {
  ...
  it('ХАРАКТЕРИЗАЦИЯ: парсер не падает и не сообщает об ошибке при 105 словах в одном блоке', () => {
    const result = parser.parse(buildDocWithNWords(105))
    expect(result.errors).toEqual([])
    expect(result.warnings).toEqual([])
    if (result.lesson?.blocks[0].type === 'vocabulary') {
      const words = result.lesson.blocks[0].words
      expect(words).toHaveLength(105)
      expect(words[97].id).toBe('05-b1-vocab-98') // 98-й элемент — ещё 2 цифры, ок
      expect(words[98].id).toBe('05-b1-vocab-99') // 99-й элемент — последний валидный по паттерну
      expect(words[99].id).toBe('05-b1-vocab-100') // 100-й элемент — уже 3 цифры!
      expect(words[104].id).toBe('05-b1-vocab-105')
    } else {
      throw new Error('ожидался блок vocabulary')
    }
  })

  it('НАЙДЕН БАГ: id от 100-го слова и далее НЕ соответствуют id-паттерну схемы shared/lesson.schema.json', () => {
    const result = parser.parse(buildDocWithNWords(105))
    if (result.lesson?.blocks[0].type !== 'vocabulary') throw new Error('ожидался блок vocabulary')
    const words = result.lesson.blocks[0].words
    const invalidIds = words.filter((w) => !ID_PATTERN.test(w.id)).map((w) => w.id)
    expect(invalidIds).toEqual(['05-b1-vocab-100', '05-b1-vocab-101', '05-b1-vocab-102', '05-b1-vocab-103', '05-b1-vocab-104', '05-b1-vocab-105'])
    // Первые 99 — валидны (ok, показывает, что граница ровно на 100-м, а не системная поломка)
    expect(words.slice(0, 99).every((w) => ID_PATTERN.test(w.id))).toBe(true)
  })

  it('НАЙДЕН БАГ (сквозной, через реальный ajv): lesson.json для vocabulary-блока со 105 словами НЕ проходит валидацию shared/lesson.schema.json', () => {
    const result = parser.parse(buildDocWithNWords(105))
    const skeleton = buildLessonSkeleton(result.lesson!, FIXED_BUILD_OPTS, result.stats)
    const valid = validateLesson(skeleton)
    expect(valid).toBe(false)
    const idErrors = (validateLesson.errors ?? []).filter((e) => e.instancePath.endsWith('/id'))
  ...
  it.skip('БАГ-репро: lesson.json для vocabulary-блока со 105 словами ДОЛЖЕН быть валиден по схеме (или парсер должен явно отклонять группы с 100+ элементами)', () => {
    const result = parser.parse(buildDocWithNWords(105))
    const skeleton = buildLessonSkeleton(result.lesson!, FIXED_BUILD_OPTS, result.stats)
...
describe('ParserService — edge-case: тема с номером 100 (переполнение 2-значного topic-номера)', () => {
  ...
  const TOPIC_ID_PATTERN = /^[0-9]{2}-[a-z0-9-]+$/

  it('ХАРАКТЕРИЗАЦИЯ: парсер принимает #TOPIC 100 без единой ошибки или предупреждения', () => {
    const result = parser.parse(DOC_TOPIC_100)
    expect(result.errors).toEqual([])
    expect(result.warnings).toEqual([])
    expect(result.lesson?.topicNumber).toBe(100)
    expect(result.lesson?.topicId).toBe('100-sotaya-tema')
    if (result.lesson?.blocks[0].type === 'vocabulary') {
      expect(result.lesson.blocks[0].words[0].id).toBe('100-b1-vocab-01')
    } else {
      throw new Error('ожидался блок vocabulary')
    }
  })

  it('НАЙДЕН БАГ: topic_id "100-sotaya-tema" не соответствует паттерну схемы (ожидается РОВНО 2 цифры в начале)', () => {
    const result = parser.parse(DOC_TOPIC_100)
    expect(TOPIC_ID_PATTERN.test(result.lesson!.topicId)).toBe(false)
  })

  it('НАЙДЕН БАГ (сквозной, через реальный ajv): lesson.json с #TOPIC 100 НЕ проходит валидацию — topic_id, а не только «лишние» id', () => {
    const result = parser.parse(DOC_TOPIC_100)
    const skeleton = buildLessonSkeleton(result.lesson!, FIXED_BUILD_OPTS, result.stats)
    const valid = validateLesson(skeleton)
    expect(valid).toBe(false)
    expect((validateLesson.errors ?? []).some((e) => e.instancePath === '/topic_id')).toBe(true)
  })
  ...
  it.skip('БАГ-репро: parse() ДОЛЖЕН либо ограничивать #TOPIC значением <=99 структурированной ошибкой на своей строке, либо lesson.json со схемой должны совпадать по паджингу', () => {
    const result = parser.parse(DOC_TOPIC_100)
    const skeleton = buildLessonSkeleton(result.lesson!, FIXED_BUILD_OPTS, result.stats)
    expect(validateLesson(skeleton)).toBe(true) // сейчас false
  })
