...
describe('CLI smoke: `npm run cli -- parse` на реальных файлах курса', () => {
  ...
  it(
    'parse файла темы 100 -> CLI завершается ненулевым кодом с ошибкой парсера (фикс переполнения id)',
    async () => {
      ...
      try {
        await execFileAsync('npm', ['run', 'cli', '--', 'parse', '--input', inputPath], {
          cwd: COMBINE_ROOT,
          timeout: 30000
        })
        throw new Error('ожидался ненулевой код возврата CLI')
      } catch (error) {
        const e = error as { code?: number; stdout?: string; stderr?: string }
        if (e.code === undefined) throw error
        expect(e.code).not.toBe(0)
        expect(`${e.stdout ?? ''}${e.stderr ?? ''}`).toMatch(/от 1 до 99/)
      } finally {
        await rm(workDir, { recursive: true, force: true })
      }
    },
